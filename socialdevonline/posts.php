...
if (isset($_GET["action"]) && $_GET["action"] == "favoris") {
    $res = add_favoris($_GET["post_id"], $_SESSION["id"]);
    if ($res) {
        header("Location:posts.php");
        exit();
    }
...
if(isset($_POST["send_comment"])){
    $post_id = $_POST["post_id"]; //champ caché ou hidden
    $user_id = $_SESSION["id"];
...
<div class="row justify-content-center">
    <div class="col-lg-7">
        <!-- Formulaire d'envoie de post -->
            <form action="" method="POST" enctype="multipart/form-data">
                <div class="mb-3">
                    ...
                    <input type="file" name="feedpicture" id="" class="form-control">
                    <textarea name="feed" id="feed" class="form-control form-control-lg" cols="30" rows="4">
                    </textarea>
                </div>
                <button type="submet" class="btn btn-primary mb-3" name="envoi">Poster</button>
            </form>
        <!-- Les posts -->
            <?php 
                $posts =  get_all_posts();
      ...
      //var_dump($user);
?>
                                       <img class="img-xs rounded-circle" src="https://bootdey.com/img/Content/avatar/avatar6.png" alt="" width=50>
                                       <!-- <img   class="img-xs rounded-circle" src="./assets/img/<?php //echo $user["profile_picture"] ?>" alt="" width=30> -->
                                       ...
                                       <div class="ml-2">
                                        ...
                                        ?>
                                          <p class="mb-0">  
                                            <a href="view-profile.php?id=<?php echo $post["user_id"] ?>">
                                            <?php 
                                                echo $user['firstname']
                                            ?>
                                            <?php 
                                          ...
                                          <p class="tx-11 text-muted">
                                            <?php
                                                echo display_time_ago($post["created"])
                                            ?>
                                          </p>
                                        </div>
                                    </div>
                          ...
                          <div class="card-body">
                            ...
                            <div class="card-footer">
                              ...
                              <a href="posts.php?action=like&post_id=<?php echo $post["post_id"] ?>" class="d-flex align-items-center text-muted mr-4 text-decoration-none">
                                    ...
                                    <?php } ?>
                                    <p class="d-md-block ms-2 my-0"><span><?= get_count_likes_by_postid($post["post_id"]) ?></span> Like</p>
                                    

                                </a>
                                <a href="" class="d-flex align-items-center text-muted ms-4 text-decoration-none">
                                <i class="fa-regular fa-comment"></i>
                                <p class="d-md-block ms-2 my-0"><span><?= get_count_comments_by_postid($post["post_id"]) ?></span> Commentaires</p>
                                </a> 

                                <a href="posts.php?action=favoris&post_id=<?php echo $post["post_id"] ?>" class="d-flex align-items-center text-muted mr-4 text-decoration-none">
                                 <p class="d-md-block ms-2 my-0" style="color: grey ;" ><span><?= get_count_favoris_by_postid($post["post_id"]) ?></span> Favoris</p>
                                    <?php if (check_if_already_favoris($post["post_id"], $_SESSION['id'])) { ?>
                                        <i class="fa-solid fa-bookmark" style="color: yellow;"></i>
                                    <?php } else { ?>
                                        <i class="fa-solid fa-bookmark"></i>
                                    <?php } ?>
                                    <a href="favoris.php?action=favoris&post_id=<?php echo $post["post_id"] ?>" style="  text-decoration: none;">
                                ...
                                <?php $user = get_user_by_id($post["user_id"]);
                                   ...
                                   ?>
                                <a href="message.php" class="d-flex align-items-center text-muted ms-4 text-decoration-none">
                                <i class="fa-regular fa-comment"></i>
                                <p class="d-md-block ms-2 my-0"></span>Message</p>
                                </a> 
