...
if (isset($_POST["edit"])) {
    if (!empty($_POST['nom']) && !empty($_POST['prenom']) && !empty($_POST['tel'])) {
        ...
        $img_folder = $_SERVER["DOCUMENT_ROOT"] . '/socialdev/assets/img/';
        // var_dump($_FILES["pp"]);
        if (!empty($_FILES["pp"])) {
                ...
                <section class="panel">
                    <div class="panel-body noradius padding-10">
                        <figure class="margin-bottom-10"><!-- image -->
                            <img style='width:100%' class="img-responsive" src="./assets/img/<?php echo $user["profile_picture"] ?>" alt="">
                        </figure><!-- /image -->
            ...
            <div class="col-md-8">
                ...
                <div id="edit">
                    <form class="form-horizontal padding-10" method="POST" action="" enctype="multipart/form-data">
                       ...
                       <a href="./delete.php?user=<?php echo $user['user_id']?>"><button type="button" class="btn btn-danger"> Supprimer le compte</button></a> 
                       <!-- <a href="supprimer.php"><option>Delete the user</option></a> -->
                    </form>
                </div>
            </div><!-- /COL 2 -->
        </div>
